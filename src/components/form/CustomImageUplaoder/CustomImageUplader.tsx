...
interface CustomImageUploaderProps<T extends FieldValues> {
  control: Control<T>;
  name: Path<T>;
  label?: string;
  maxFiles?: number;
...
const CustomImageUploader = <T extends FieldValues>({
  control,
  name,
  label = "Upload Image",
  maxFiles = 1,
    ...
    rules: {
      validate: {
        minFiles: (val: ImageFieldValue) => {
          ...
          return !required || totalImages >= 1 || "At least one image is required"; // Fallback, overridden by Zod
        },
        maxFiles: (val: ImageFieldValue) => {
          const totalImages = val.newImages.length + (existingImages.length - val.removedExisting.length);
          return totalImages <= maxFiles || `Cannot have more than ${maxFiles} image${maxFiles > 1 ? "s" : ""}`;
        },
  ...
  const handleImageFiles = (files: FileList) => {
    const selectedFiles = Array.from(files).filter((file) => file.type.startsWith("image/") && file.size <= maxFileSize * 1024 * 1024);
  ...
  const removeImage = (index: number) => {
    ...
    if (index < existingCount) {
      ...
      const newImages = value.newImages.filter((_, idx: number) => idx !== newImageIndex);
